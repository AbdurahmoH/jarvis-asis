...
def test_mission_streaming_via_task_events(stream_backend, settings):
    ...
    # Аудит 1b: исходная формулировка с союзом «и» восстановлена — сплит
    # составных команд теперь валидируется решением роутера, миссия не
    # разбивается на части.
    goal = "Напиши развёрнутое эссе о том, как паровые машины изменили "            "промышленность, транспорт и жизнь городов в девятнадцатом веке"
